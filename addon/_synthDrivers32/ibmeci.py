...
from . import _ibmeci
EciLangs = _ibmeci.ECILanguageDialect
ECIParam = _ibmeci.ECIParam
ECIVoiceParam = _ibmeci.ECIVoiceParam
...
class SynthDriver(synthDriverHandler.SynthDriver):
	# All settings needed for synthDriverHost32
	# Note: Strings are not translated in 32-bit side, translations come from 64-bit
	supportedSettings = [
		SynthDriver.VoiceSetting(),
		SynthDriver.VariantSetting(),
		SynthDriver.RateSetting(),
		BooleanDriverSetting("rateBoost", "Rate boost", True),
		SynthDriver.PitchSetting(),
		SynthDriver.InflectionSetting(),
		SynthDriver.VolumeSetting(),
		NumericDriverSetting("hsz", "Head size", False),
		NumericDriverSetting("rgh", "Roughness", False),
		NumericDriverSetting("bth", "Breathiness", False),
		BooleanDriverSetting("backquoteVoiceTags", "Enable backquote voice tags", False),
		BooleanDriverSetting("ABRDICT", "Enable abbreviation expansion", False),
		BooleanDriverSetting("phrasePrediction", "Enable phrase prediction", False),
		DriverSetting("pauseMode", "Pauses", defaultVal="2"),
		BooleanDriverSetting("sendParams", "Always Send Current Speech Settings", False, defaultVal=True),
		DriverSetting('sampleRate', 'Sample Rate', defaultVal='1'),
	]
	supportedCommands = {
		IndexCommand,
	...
	def _set_rateBoost(self, enable):
		if enable != getattr(self, '_rateBoost', True):
			rate = self.rate
	...
	def _get_rate(self):
		...
		if getattr(self, '_rateBoost', True): val=int(round(val/self.RATE_BOOST_MULTIPLIER))
	...
	def _get_availableSamplerates(self):
		rates = {}
		rates["0"] = StringParameterInfo("0", _("8 kHz"))
		rates["1"] = StringParameterInfo("1", _("11 kHz"))
		if _ibmeci.isIBM:
			rates["2"] = StringParameterInfo("2", _("22 kHz"))
		return rates
