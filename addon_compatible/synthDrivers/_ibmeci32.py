...
string_types = (str,)  # Python 3 compatible - no six needed
from logHandler import log

from . import _ibmeci32Impl
from ._ibmeci32Impl import ECILanguageDialect as EciLangs, ECIParam, ECIVoiceParam
...
class SynthDriver(synthDriverHandler.SynthDriver):
	...
	@classmethod
	def check(cls):
		return _ibmeci32Impl.eciCheck()

	def __init__(self):
		_ibmeci32Impl.initialize(self._onIndexReached, self._onDoneSpeaking)
		...
		log.debug("Using IBMTTS version %s" % _ibmeci32Impl.eciVersion())
	...
	def speak(self,speechSequence):
		...
		charmode=False
		if self._ABRDICT:
			_ibmeci32Impl.setParam(ECIParam.eciDictionary, 0)
		else:
			_ibmeci32Impl.setParam(ECIParam.eciDictionary, 1)
		embeds=b''
		if self._phrasePrediction:
			...
			embeds+=b"`pp0 "
		if self._sendParams:
			embeds+=b"`vv%d `vs%d " % (_ibmeci32Impl.getVParam(ECIVoiceParam.eciVolume), _ibmeci32Impl.getVParam(ECIVoiceParam.eciSpeed))
		outlist.append((_ibmeci32Impl.speak, (embeds,)))
		speechSequence= self.combine_adjacent_strings(speechSequence)
		for item in speechSequence:
			if isinstance(item, string_types):
				s = self.processText(item)
				outlist.append((_ibmeci32Impl.speak, (s,)))
				last = s
			elif isinstance(item,IndexCommand):
				outlist.append((_ibmeci32Impl.index, (item.index,)))
				...
				if l:
					if item.lang != self.speakingLanguage and item.lang != self.speakingLanguage[0:2]:
						outlist.append((_ibmeci32Impl.speak, (l,)))
						self.speakingLanguage=item.lang
						self.updateEncoding(l)
				else:
					outlist.append((_ibmeci32Impl.speak, (langsAnnotations[defaultLanguage],)))
					self.speakingLanguage = defaultLanguage
			elif isinstance(item,CharacterModeCommand):
				outlist.append((_ibmeci32Impl.speak, (b"`ts1" if item.state else b"`ts0",)))
				if item.state:
					charmode=True
				...
				pFactor = int(pFactor)
				outlist.append((_ibmeci32Impl.speak, (b' `p%d '%(pFactor),)))
			elif type(item) in self.PROSODY_ATTRS:
				val = max(0, min(item.newValue, 100))
				if type(item) == RateCommand: val = self.percentToRate(val)
				outlist.append((_ibmeci32Impl.setProsodyParam, (self.PROSODY_ATTRS[type(item)], val)))
			else:
		...
		if last and last[-1] not in punctuation:
			...
			if self._pause_mode >= 1:
				if outlist[-2][0] == _ibmeci32Impl.setProsodyParam: outlist.insert(-2, (_ibmeci32Impl.speak, (b'`p1 ',)))
				else: outlist.append((_ibmeci32Impl.speak, (b'`p1 ',)))
		if charmode:
			outlist.append((_ibmeci32Impl.speak, (b"`ts0",)))
		outlist.append((_ibmeci32Impl.setEndStringMark, ()))
		outlist.append((_ibmeci32Impl.synth, ()))
		_ibmeci32Impl.eciQueue.put(outlist)
		_ibmeci32Impl.process()
	...
	def processText(self,text):
		...
		curLang = _ibmeci32Impl.params[_ibmeci32Impl.ECIParam.eciLanguageDialect]
		if _ibmeci32Impl.isIBM:
	...
	def _get_rate(self):
		val = _ibmeci32Impl.getVParam(ECIVoiceParam.eciSpeed)
	...
	def _set_rate(self,val):
		...
		self._rate = val
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciSpeed, val)

	def _get_pitch(self):
		return _ibmeci32Impl.getVParam(ECIVoiceParam.eciPitchBaseline)

	def _set_pitch(self,vl):
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciPitchBaseline,vl)

	def _get_volume(self):
		return _ibmeci32Impl.getVParam(ECIVoiceParam.eciVolume)

	def _set_volume(self,vl):
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciVolume,int(vl))

	def _set_inflection(self,vl):
		vl = int(vl)
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciPitchFluctuation,vl)

	def _get_inflection(self):
		return _ibmeci32Impl.getVParam(ECIVoiceParam.eciPitchFluctuation)

	def _set_hsz(self,vl):
		vl = int(vl)
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciHeadSize,vl)

	def _get_hsz(self):
		return _ibmeci32Impl.getVParam(ECIVoiceParam.eciHeadSize)

	def _set_rgh(self,vl):
		vl = int(vl)
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciRoughness,vl)

	def _get_rgh(self):
		return _ibmeci32Impl.getVParam(ECIVoiceParam.eciRoughness)

	def _set_bth(self,vl):
		vl = int(vl)
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciBreathiness,vl)

	def _get_bth(self):
		return _ibmeci32Impl.getVParam(ECIVoiceParam.eciBreathiness)

	def _getAvailableVoices(self):
		o = OrderedDict()
		for name in os.listdir(_ibmeci32Impl.ttsPath):
			if name.lower().endswith('.syn'):
				info = _ibmeci32Impl.langs[name.lower()[:3]]
				o[str(info[0])] = VoiceInfo(str(info[0]), info[1], info[2])
		return o

	def _get_voice(self):
		return str(_ibmeci32Impl.params[_ibmeci32Impl.ECIParam.eciLanguageDialect])
	def _set_voice(self,vl):
		_ibmeci32Impl.setVoice(int(vl))
	...
	def _set_sampleRate(self, val):
		val = int(val)
		if val == 2 and not _ibmeci32Impl.isIBM:
			val = 1
		self._sample_rate = val
		if _ibmeci32Impl.player is not None:
			self.cancel()
		_ibmeci32Impl.setParam(_ibmeci32Impl.ECIParam.eciSampleRate, val)
		_ibmeci32Impl.player = _ibmeci32Impl.createPlayer(val)
	...
	def _get_lastIndex(self):
		#fix?
		return _ibmeci32Impl.lastindex
	...
	def _set_variant(self, v):
		...
		_ibmeci32Impl.setVariant(int(v))
		_ibmeci32Impl.setVParam(ECIVoiceParam.eciSpeed, self._rate)
