...
try:
	...
	_translatorFunction = addonHandler.getTranslation
except (ImportError, AttributeError):
	_translatorFunction = lambda x: x

import builtins
builtins._ = _translatorFunction
...
def setPathsFromConfig():
	global ttsPath, dllName, _configSet
	# If setDllConfig was already called, ttsPath and dllName are already set
	if _configSet:
		return
	# Read nvda.ini directly as text to get dllName and TTSPath from [ibmeci] section
	try:
		configPath = path.abspath(path.join(path.dirname(__file__), "..", "..", "..", "nvda.ini"))
		if path.exists(configPath):
			with open(configPath, 'r', encoding='utf-8') as f:
				content = f.read()
			# Find [ibmeci] section and extract dllName and TTSPath
			in_ibmeci = False
			for line in content.split('\n'):
				...
				if line == '[ibmeci]':
					...
					continue
				if in_ibmeci:
					if line.startswith('[') and line != '[ibmeci]':
...
def setDllConfig(dllNameParam, TTSPathParam):
	global ttsPath, dllName, _configSet
	dllName = dllNameParam
	if not path.isabs(TTSPathParam):
		ttsPath = path.abspath(path.join(path.abspath(path.dirname(__file__)), TTSPathParam))
	else:
		ttsPath = TTSPathParam
	_configSet = True
...
def loadEciLibrary():
	...
	# Check if this is IBM ECI if fileUtils is available
	if getFileVersionInfo is not None and path.exists(eciLibPath):
		try:
		...
		# Assume IBM ECI if we can't check
		isIBM = True
...
def eciNew():
	...
	# In synthDriverHost32, always use language fallback to avoid config.conf RPYC calls
	langVoice = getVoiceByLanguage(languageHandler.getLanguage())[0]
	handle=eci.eciNewEx(langVoice)
	for i in ECIVoiceParam.params:
...
# Global function to get available pause modes (for synthDriverHost32 RPYC exposure)
_pauseModesGlobal = {
...
def getAvailablePausemodes():
	"""Returns available pause modes for IBMTTS (called via RPYC from 64-bit)."""
	return _pauseModesGlobal

# Global function to get available sample rates (for synthDriverHost32 RPYC exposure)
_sampleRatesGlobal = {
	"0": StringParameterInfo("0", "8 kHz"),
	"1": StringParameterInfo("1", "11 kHz")
}
_sampleRatesGlobalWith22 = {
	"0": StringParameterInfo("0", "8 kHz"),
	"1": StringParameterInfo("1", "11 kHz"),
	"2": StringParameterInfo("2", "22 kHz")
}

def getAvailableSamplerates():
	"""Returns available sample rates for IBMTTS (called via RPYC from 64-bit)."""
	# Only include 22kHz if isIBM is True
	global isIBM
	if isIBM:
		return _sampleRatesGlobalWith22
	else:
		return _sampleRatesGlobal

def getAvailableVoices():
	"""Returns available voices for IBMTTS (called via RPYC from 64-bit).
	Returns a list of tuples: (ID, name, language)
	"""
	global ttsPath, langs
	result = []
	try:
		from os import listdir
		for name in listdir(ttsPath):
			if name.lower().endswith('.syn'):
				info = langs[name.lower()[:3]]
				result.append((str(info[0]), str(info[1]), str(info[2])))
	except Exception:
		pass
	return result

# Functions for RPYC parameter access from 64-bit
_paramStorage = {
	"rateBoost": False,
	"inflection": 50,
	"hsz": 50,
...
def getParam(name):
	"""Get a parameter value (called via RPYC from 64-bit)."""
	global _paramStorage, isIBM
	if name == "isIBM":
		return isIBM
	return _paramStorage.get(name)

def setParam(name, value):
	"""Set a parameter value (called via RPYC from 64-bit)."""
	_paramStorage[name] = value
